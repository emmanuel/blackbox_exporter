...
func NewTCP(name string, module *config.Module) Prober {
	prober := &tcpProber{
		name:   name,
		module: module,
	}
...
func (this *tcpProber) Probe(ctx context.Context, target string, registry *prometheus.Registry, logger *zap.Logger) error {
	m := metrics.NewTCPProbeMetrics(registry)
	log := logger.With(zap.String("module", this.name))
	m.SetPhaseDurationMetric(metrics.ProbeTCPDurationPhaseResolve, 0)

	log.Info("probing TCP target")
